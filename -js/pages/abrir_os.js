document
  ...
  .addEventListener("submit", async function (event) {
    ...
    // 1. Recuperar dados da sessão ativa no navegador (para pegar o ID do usuário logado de verdade)
    const sessaoStr = localStorage.getItem("sgm_sessao");
    const sessao = sessaoStr ? JSON.parse(sessaoStr) : null;
    const idUsuarioLogado = sessao ? sessao.id : 1; // Fallback para 1 se não achar
    ...
    // Valores padrão para campos que foram removidos da tela visual
    ...
    try {
      ...
      const novaAbertura = {
        ...
        id_usuario_responsavel: idUsuarioLogado, // Atribui ao próprio usuário ou deixa o ADM responsável inicial
